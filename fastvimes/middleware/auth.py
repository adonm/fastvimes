...
class AuthenticationUI:
        ...
        with ui.card().classes("w-full max-w-md mx-auto mt-16"):
            ...
            with ui.column().classes("w-full gap-4"):
                ...
                with ui.row().classes("w-full justify-center"):
                    ui.button(
                        "Login",
                        on_click=lambda: self._handle_login(
    ...
    def render_user_info(self, user_data: dict[str, Any]):
        ...
        with ui.row().classes("items-center gap-2"):
            ...
            ui.label(f"{user_data['username']} ({user_data['role']})").classes(
                "text-sm"
            )
            self.render_logout_button()

    def check_authentication(self) -> dict[str, Any] | None:
        """Check if user is authenticated."""
        # This would typically get token from browser storage
        # For demo purposes, we'll return a mock user
        return {"username": "demo_user", "role": "admin", "token": "demo_token"}
