...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: FastAPI, settings: FastVimesSettings):
        ...
        self.oauth.register(
            ...
            client_kwargs={
                'scope': ' '.join(self.auth_config.oauth_scopes)
            }
        )
    ...
    async def dispatch(self, request: StarletteRequest, call_next) -> Response:
        ...
        if self.auth_config.require_auth_for_write and request.method in ['POST', 'PUT', 'DELETE']:
    ...
    async def _get_current_user(self, request: StarletteRequest) -> Optional[AuthUser]:
    ...
    def create_jwt_token(self, user_data: Dict[str, Any]) -> str:
        ...
        return jwt.encode({'alg': self.auth_config.jwt_algorithm}, payload, self.auth_config.jwt_secret_key)
...
def setup_auth_routes(app: FastAPI, auth_middleware: AuthMiddleware):
    ...
    async def login(request: Request):
        ...
        client = auth_middleware.oauth.create_client(auth_middleware.auth_config.oauth_provider)
