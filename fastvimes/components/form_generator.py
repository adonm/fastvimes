...
class FormGenerator:
        ...
        if 'varchar' in column_type.lower() or 'text' in column_type.lower():
            if 'text' in column_type.lower():
                ...
                ui.textarea(
                    ...
                    on_change=lambda value, col=column_name: self._update_form_data(col, value)
                ).classes('w-full')
            else:
                # Regular text input
                ui.input(
    ...
    def _save_record(self):
        ...
        try:
            ...
            clean_data = {k: v for k, v in self.form_data.items() 
                         if v is not None and v != ''}
            
            result = self.api_client.create_record(self.table_name, clean_data)
            ui.notify(f'Record created successfully! ID: {result.get("id")}', type='positive')
            self._clear_form()
        except Exception as e:
