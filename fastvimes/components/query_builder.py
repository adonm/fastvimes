...
class QueryBuilder:
            ...
            with ui.row().classes("w-full"):
                ...
                ui.select(
                    ...
                    on_change=lambda value: setattr(
                        self, "sort_direction", value.value
                    ),
                ).classes("w-20")
    ...
    def _render_limit_control(self):
        ...
        with ui.column().classes("min-w-32"):
            ...
            ui.number(
                ...
                on_change=lambda value: setattr(
                    self, "limit_value", value.value or 100
                ),
            ).classes("w-full")
    ...
    def _render_filter_controls(self, schema: list[dict[str, Any]]):
        ...
        with ui.expansion("Filters", icon="filter_list").classes("w-full"):
            with ui.column().classes("w-full"):
                # Add filter button
                ui.button(
                    "Add Filter", icon="add", on_click=lambda: self._add_filter(schema)
                ).props("dense outline")
    ...
    def _render_filter_row(
        self, index: int, filter_config: dict[str, Any], schema: list[dict[str, Any]]
    ):
        ...
        with ui.row().classes("w-full items-center gap-2"):
            ...
            ui.select(
                ...
                on_change=lambda value, idx=index: self._update_filter(
                    idx, "column", value.value
                ),
            ).classes("w-40")
            ...
            ui.input(
                ...
                placeholder="Value",
                on_change=lambda value, idx=index: self._update_filter(
                    idx, "value", value.value
                ),
            ).classes("flex-grow")

            # Remove filter button
            ui.button(
                icon="remove", on_click=lambda idx=index: self._remove_filter(idx)
    ...
    def _render_query_controls(self):
        ...
        with ui.row().classes("w-full items-center gap-4 mt-4"):
            ...
            ui.input(value=rql_query, placeholder="Generated RQL query...").classes(
                "flex-grow"
            ).props("outlined readonly")

            # Execute button
            ui.button(
                "Execute Query", icon="play_arrow", on_click=self._execute_query
            ).props("color=primary")
    ...
    def _execute_query(self):
        ...
        try:
            ...
            result = self.api_client.get_table_data(
                self.table_name, rql_query=rql_query, limit=self.limit_value
            )
