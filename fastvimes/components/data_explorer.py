...
class DataExplorer:
    ...
    def render(self):
        ...
        if not self.schema or not self.data_result:
            ui.label(f"Error loading data for {self.table_name}").classes(
                "text-red-500"
            )
            return
    ...
    def _render_header(self):
        ...
        with ui.row().classes("w-full justify-between items-center mb-4"):
            ...
            with ui.row().classes("gap-2"):
                ui.button("Add Record", icon="add", on_click=self._add_record).props(
                    "dense"
                )
                ui.button(
                    "Toggle Edit Mode", icon="edit", on_click=self._toggle_edit_mode
                ).props("dense outline")
                ui.button("Refresh", icon="refresh", on_click=self._refresh_data).props(
                    "dense outline"
                )
                ui.button(
                    "Export CSV", icon="download", on_click=self._export_csv
                ).props("dense outline")
    ...
    def _render_filters(self):
        ...
        with ui.expansion("Filters", icon="filter_list"):
            ...
            with ui.row().classes("w-full justify-end mt-2"):
                ui.button(
                    "Apply Filters", icon="filter_list", on_click=self._apply_filters
                ).props("dense")
                ui.button(
                    "Clear Filters", icon="clear", on_click=self._clear_filters
                ).props("dense outline")
        ...
        with ui.column().classes("min-w-48"):
            ...
            if "varchar" in column_type.lower() or "text" in column_type.lower():
                ui.input(
                    ...
                    on_change=lambda value, col=column_name: self._update_filter(
                        col, value
                    ),
                ).classes("w-full")
    ...
    def _render_bulk_operations(self):
        ...
        with ui.row().classes("w-full p-2 bg-blue-50 rounded mb-2"):
            ...
            with ui.row().classes("gap-2 ml-auto"):
                ui.button("Edit Selected", icon="edit", on_click=self._bulk_edit).props(
                    "dense"
                )
                ui.button(
                    "Delete Selected", icon="delete", on_click=self._bulk_delete
                ).props("dense color=negative")
                ui.button(
                    "Clear Selection", icon="clear", on_click=self._clear_selection
                ).props("dense outline")
    ...
    def _render_data_table(self):
        ...
        with ui.row().classes("w-full items-center justify-between mb-2"):
            ui.label(
                f"Showing {len(rows)} of {self.data_result['total_count']} records"
    ...
    def _render_pagination(self):
        ...
        with ui.row().classes("w-full justify-center"):
            # Previous button
            ui.button("Previous", on_click=self._previous_page).props(
                "outline"
            ).set_enabled(self.current_page > 0)

            # Page info
            ui.label(f"Page {self.current_page + 1} of {total_pages}").classes("mx-4")

            # Next button
            ui.button("Next", on_click=self._next_page).props("outline").set_enabled(
        ...
        with ui.dialog().props("max-width=500px") as dialog:
            with ui.card():
                ...
                with ui.row().classes("w-full justify-end mt-4"):
                    ...
                    ui.button(
                        "Save", on_click=lambda: self._save_new_record(dialog, inputs)
                    )
    ...
    def _bulk_edit(self):
        ...
        with ui.dialog().props("max-width=500px") as dialog:
            with ui.card():
                ui.label(f"Edit {len(self.selected_rows)} Records").classes(
                    "text-h6 mb-4"
                )
                ...
                for column in self.schema:
                    ...
                    with ui.row().classes("w-full"):
                        ...
                        if "varchar" in col_type.lower() or "text" in col_type.lower():
                            inputs[col_name] = ui.input(
                                placeholder="Leave empty to keep current value"
                            ).classes("flex-1")
                        elif "int" in col_type.lower():
                            inputs[col_name] = ui.number(
                                placeholder="Leave empty to keep current value"
                            ).classes("flex-1")
                        elif "bool" in col_type.lower():
                            inputs[col_name] = ui.select(
                                ["true", "false"], placeholder="Keep current value"
                            ).classes("flex-1")
                        else:
                            inputs[col_name] = ui.input(
                                placeholder="Leave empty to keep current value"
                ...
                with ui.row().classes("w-full justify-end mt-4"):
                    ui.button("Cancel", on_click=dialog.close).props("outline")
                    ui.button(
                        "Update", on_click=lambda: self._save_bulk_edit(dialog, inputs)
                    )
    ...
    def _bulk_delete(self):
        ...
        with ui.dialog() as dialog:
            with ui.card():
                ui.label(f"Delete {len(self.selected_rows)} Records").classes(
                    "text-h6 mb-4"
                )
                ui.label(
                ...
                with ui.row().classes("w-full justify-end"):
                    ui.button("Cancel", on_click=dialog.close).props("outline")
                    ui.button(
                        "Delete", on_click=lambda: self._confirm_bulk_delete(dialog)
    ...
    def _export_csv(self):
        ...
        try:
            ...
            csv_data = self.api_client.get_table_data(
                self.table_name, rql_query=rql_query, format="csv"
            )
