...
class RQLToSQLConverter:
        ...
        if isinstance(parsed_rql, dict):
            ...
            if conditions:
                ...
                for condition in conditions[1:]:
                    where_condition = exp.And(this=where_condition, expression=condition)
        ...
        if isinstance(rql_node, dict):
            ...
            if operator == 'and':
                ...
                if conditions:
                    where_condition = conditions[0]
                    for condition in conditions[1:]:
                        where_condition = exp.Or(this=where_condition, expression=condition)
                    query = query.where(where_condition)
            ...
            elif operator in ['eq', 'ne', 'lt', 'le', 'gt', 'ge', 'contains', 'in', 'out']:
    ...
    def _convert_rql_condition(self, rql_node, params: List[Any]) -> Tuple[Optional[exp.Expression], List[Any]]:
...
def convert_rql_to_sql(table_name: str, rql_query: str, dialect: str = "duckdb") -> Tuple[str, List[Any]]:
