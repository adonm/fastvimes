...
class SchemaValidator:
    """Validates column names and operations against table schemas."""

    def __init__(self, table_schema: dict[str, Any]):
        """Initialize with table schema containing column information."""
        self.table_schema = table_schema
        self.valid_columns = (
            set(table_schema.get("columns", {}).keys()) if table_schema else set()
        )

    def is_valid_column(self, column_name: str) -> bool:
        """Check if column name exists in the table schema."""
        return column_name in self.valid_columns
...
class RQLSQLGlotTranslator:
    ...
    OPERATORS = {
        ...
        "in": lambda col, val: exp.In(
            this=col,
            expressions=[
                _value_to_literal(v) for v in (val if isinstance(val, list) else [val])
            ],
        ),
        ...
        "contains": lambda col, val: exp.Like(
            this=col, expression=exp.Literal.string(f"%{val}%")
        ),
        "excludes": lambda col, val: exp.Not(
            this=exp.Like(this=col, expression=exp.Literal.string(f"%{val}%"))
        ),
    }

    def __init__(self, schema_validator=None):
        """Initialize with optional schema validator for column name validation."""
    ...
    def parse_rql_query(self, rql_string: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Re-raise validation errors (e.g., invalid column names)
    ...
    def _process_query_node(self, node: dict[str, Any], result: dict[str, Any]):
        ...
        if name in ["and", "or"]:
            ...
            if len(args) >= 2:
                ...
                # Validate column name against schema if validator is provided
                if self.schema_validator and not self.schema_validator.is_valid_column(
                    field
                ):
                ...
                for arg in args:
                    if isinstance(arg, tuple) and len(arg) == 2:
                        ...
                        if (
                            ...
                            and not self.schema_validator.is_valid_column(field)
                        ):
                            raise ValueError(f"Invalid column name in sort: {field}")
                        ...
                        if ascending:
                            ...
                            sorts.append(
                                exp.Ordered(this=Column(this=field), desc=True)
                            )
                    elif isinstance(arg, str):
                        # Validate column name against schema if validator is provided
                        if (
                            self.schema_validator
                            and not self.schema_validator.is_valid_column(arg)
                        ):
                            raise ValueError(f"Invalid column name in sort: {arg}")
            ...
            for field in args:
                ...
                if self.schema_validator and not self.schema_validator.is_valid_column(
                    ...
                    raise ValueError(f"Invalid column name in select: {field}")
    ...
    def build_select_query(self, table_name: str, rql_data: dict[str, Any]) -> str:
    ...
    def parse_query_params(self, query_params: dict[str, Any]) -> str:
        ...
        if "sort" in query_params:
            ...
            if isinstance(sort_value, str):
                ...
                if sort_fields:
                    sort_args = ",".join(
                        [f"({direction},{field})" for direction, field in sort_fields]
                    )
