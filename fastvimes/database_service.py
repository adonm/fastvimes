...
class DatabaseService:
    ...
    def _discover_tables(self) -> None:
        ...
        try:
            ...
            for table_name in table_names:
                ...
                schema_result = self._connection.execute(
                ...
                self._tables[table_name] = {"name": table_name, "columns": columns}
    ...
    def execute_rql_query(
        self, table_name: str, query_params: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """Execute an RQL query and return results."""
        try:
            from .rql_sqlglot import (
                RQLSQLGlotTranslator,
                SchemaValidator,
            )

            # Get table schema for validation
            table_schema = self.get_table_schema(table_name)
            schema_validator = SchemaValidator(table_schema)

            # Check if we have a direct RQL string
            if "_rql_string" in query_params:
                translator = RQLSQLGlotTranslator(schema_validator=schema_validator)
                sql = translator.translate_rql_to_sql(
                    table_name, query_params["_rql_string"]
                )
            else:
                # Use translator with schema validation for safety
                translator = RQLSQLGlotTranslator(schema_validator=schema_validator)
                rql_string = translator.parse_query_params(query_params)
                sql = translator.translate_rql_to_sql(table_name, rql_string)

            logger.debug(f"Generated SQL: {sql}")
            return self.execute_query(sql)
        except ValueError as e:
            # Re-raise validation errors as user-friendly messages
            logger.warning(f"Invalid RQL query: {e}")
    ...
    def update_record(
        self, table_name: str, data: dict[str, Any], where_params: dict[str, Any]
    ) -> int:
        ...
        if "_rql_string" in where_params:
            from .rql_sqlglot import RQLSQLGlotTranslator, SchemaValidator

            rql_string = where_params["_rql_string"]
            schema_validator = SchemaValidator({"columns": schema["columns"]})
            translator = RQLSQLGlotTranslator(schema_validator=schema_validator)
            ...
            for col, value in where_params.items():
                if col not in schema["columns"]:
                    raise ValueError(
                        f"Where column '{col}' does not exist in table '{table_name}'"
                    )
