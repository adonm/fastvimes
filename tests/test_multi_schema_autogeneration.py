...
class TestMultiSchemaAutogeneration:
    ...
    def test_schema_introspection_works_with_any_schema(self, multi_schema_db):
        ...
        for expected_table in service._test_expected_tables:
            assert expected_table in table_names, f"Expected table {expected_table} not found in {table_names}"
        ...
        for table_name in service._test_expected_tables:
            ...
            for col in schema:
                ...
                if any(t in col_type for t in ['INT', 'DECIMAL', 'FLOAT', 'DOUBLE', 'NUMERIC']):
                    ...
                    try:
                        result = service.get_table_data(table_name, rql_query=f"gt({col_name},0)")
                        assert 'data' in result
                    except Exception:
                    ...
                    try:
                        result = service.get_table_data(table_name, rql_query=f"ne({col_name},'')")
                        assert 'data' in result
                    except Exception:
                    ...
                    try:
                        result = service.get_table_data(table_name, rql_query=f"eq({col_name},true)")
                        assert 'data' in result
                    except Exception:
        ...
        for table_name in service._test_expected_tables:
            ...
            if test_data:
                ...
                try:
                    ...
                    has_id = any(key.endswith('_id') or key == 'id' for key in result.keys())
                    has_success = 'created' in str(result) or 'message' in result
                    assert has_id or has_success, f"Expected primary key or success message in result: {result}"
