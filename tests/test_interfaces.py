...
class TestScenarios:
    ...
    def get_sample_data() -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"id": 4, "name": "David", "email": "david@example.com", "age": 22, "active": True},
        ]
...
class TestPythonAPI:
    ...
    def test_08_create_record(self, app_with_data):
        ...
        new_user = {"name": "Eve", "email": "eve@example.com", "age": 28, "active": True}
        ...
        eve_users = app_with_data.db_service.execute_rql_query("users", {"eq(name,Eve)": None})
    ...
    def test_10_error_handling(self, app_with_data):
        ...
        with pytest.raises(Exception):
            query_params = {"invalid_syntax": None}
...
class TestCLIInterface:
    ...
    def setup_test_db(self, db_path: str):
        ...
        for user in sample_data[3:]:  # Add David (init already has Alice, Bob, Charlie)
            ...
            subprocess.run(
                ["uv", "run", "fastvimes", "post", "users", 
                 "--data", user_json, "--db", db_path],
                capture_output=True, text=True
            )
    ...
    def test_04_cli_rql_filters(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["uv", "run", "fastvimes", "get", "users", 
                 "--eq", "active,true", "--db", db_path],
                capture_output=True, text=True
            )
            assert result.returncode == 0
    ...
    def test_05_cli_create_record(self):
        ...
        try:
            ...
            new_user = {"name": "Frank", "email": "frank@example.com", "age": 45, "active": True}
            
            result = subprocess.run(
                ["uv", "run", "fastvimes", "post", "users", 
                 "--data", json.dumps(new_user), "--db", db_path],
                capture_output=True, text=True
            )
            assert result.returncode == 0
...
class TestHTTPInterface:
    ...
    def test_08_http_create_record(self, app_with_data):
        ...
        new_user = {"name": "Grace", "email": "grace@example.com", "age": 27, "active": True}
