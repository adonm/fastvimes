...
class TestUIComponentsAPIIntegration:
    ...
    @pytest.mark.xfail(
        reason="API endpoint design issue with request body parsing - needs FastAPI Body() parameter fix"
    )
    def test_form_generator_http_integration(self, test_server):
        ...
        response = test_server.put(
            f"/v1/data/users?eq(id,{record_id})", json=update_data
        )
        assert response.status_code == 200
