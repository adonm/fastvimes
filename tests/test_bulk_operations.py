...
class TestBulkOperationsCore:
    ...
    def sample_data(self):
        ...
        return [
            {"id": 1001, "name": "Alice Johnson", "email": "alice.test@example.com", "age": 30, "active": True, "department": "Engineering", "created_at": "2024-01-01 10:00:00"},
            {"id": 1002, "name": "Bob Smith", "email": "bob.test@example.com", "age": 25, "active": True, "department": "Marketing", "created_at": "2024-01-01 11:00:00"},
            {"id": 1003, "name": "Charlie Brown", "email": "charlie.test@example.com", "age": 35, "active": False, "department": "Sales", "created_at": "2024-01-01 12:00:00"}
        ]
    ...
    def test_bulk_insert_from_json_file(self, db_service, sample_data):
        ...
        try:
            ...
            inserted_emails = [row["email"] for row in result["data"] if row["email"] in ["alice.test@example.com", "bob.test@example.com", "charlie.test@example.com"]]
    ...
    def test_bulk_upsert_from_file(self, db_service, sample_data):
        ...
        # Insert initial data
        initial_data = [
            {"id": 2001, "name": "Alice Original", "email": "alice.old@example.com", "age": 28, "active": True, "department": "Engineering", "created_at": "2024-01-01 10:00:00"},
            {"id": 2002, "name": "Bob Original", "email": "bob.old@example.com", "age": 23, "active": True, "department": "Marketing", "created_at": "2024-01-01 11:00:00"}
        ]
        ...
        try:
            ...
            upsert_data = [
                {"id": 2001, "name": "Alice Updated", "email": "alice.updated@example.com", "age": 30, "active": True, "department": "Engineering", "created_at": "2024-01-01 10:00:00"},  # Update
                {"id": 2003, "name": "Charlie New", "email": "charlie.new@example.com", "age": 35, "active": False, "department": "Sales", "created_at": "2024-01-01 12:00:00"}       # Insert
            ]
            ...
            try:
                ...
                charlie_records = [row for row in all_data["data"] if row["name"] == "Charlie New"]
    ...
    def test_bulk_operations_with_multiple_key_columns(self, db_service):
        ...
        try:
            ...
            upsert_data = [
                {"category": "books", "item_id": 1, "name": "Book A Updated", "price": 22.00},  # Update
                {"category": "books", "item_id": 3, "name": "Book C", "price": 30.00}           # Insert
            ]
            ...
            try:
                ...
                book_a_records = [row for row in all_data["data"] 
                ...
                book_c_records = [row for row in all_data["data"] 
...
class TestBulkOperationsErrorHandling:
    ...
    def test_bulk_insert_schema_mismatch(self, db_service):
        ...
        invalid_data = [
            {"invalid_column": "value", "another_invalid": 123}
        ]
...
class TestBulkOperationsWithMultipleSchemas:
    ...
    def test_bulk_operations_work_with_any_schema(self, schema_db_service):
        ...
        if schema_type == 'ecommerce':
            test_data = [
                {"product_id": 1, "name": "Widget A", "category": "widgets", "price": 19.99, "stock": 100},
                {"product_id": 2, "name": "Widget B", "category": "widgets", "price": 29.99, "stock": 50}
            ]
        elif schema_type == 'blog':
            test_data = [
                {"post_id": 1, "title": "First Post", "content": "Hello World", "author": "admin", "created_at": "2024-01-01"},
                {"post_id": 2, "title": "Second Post", "content": "Another post", "author": "user", "created_at": "2024-01-02"}
            ]
        elif schema_type == 'inventory':
            test_data = [
                {"warehouse_id": "WH1", "item_code": "A001", "quantity": 100, "location": "A1"},
                {"warehouse_id": "WH1", "item_code": "A002", "quantity": 50, "location": "A2"}
            ]
        ...
        try:
            # Test bulk insert
            records_inserted = db_service.bulk_insert_from_file(table_name, temp_file, "json")
            assert records_inserted == 2
            
            # Test bulk upsert
            result = db_service.bulk_upsert_from_file(table_name, temp_file, key_columns, "json")
            assert "inserted" in result
            assert "updated" in result
            
            # Test bulk delete
            records_deleted = db_service.bulk_delete_from_file(table_name, temp_file, key_columns, "json")
    ...
    def test_bulk_operations_preserve_schema_constraints(self, schema_db_service):
        ...
        if schema_type == 'ecommerce':
            valid_data = [{"product_id": 1, "name": "Valid Product", "category": "test", "price": 10.00, "stock": 1}]
        elif schema_type == 'blog':
            valid_data = [{"post_id": 1, "title": "Valid Post", "content": "Content", "author": "test", "created_at": "2024-01-01"}]
        elif schema_type == 'inventory':
            valid_data = [{"warehouse_id": "WH1", "item_code": "TEST", "quantity": 1, "location": "TEST"}]
