...
class TestRQLFiltering:
    ...
    def test_string_filtering(self, db_service):
        ...
        result = db_service.get_table_data('users', rql_query='eq(department,Engineering)')
...
class TestRQLSelection:
    ...
    def test_select_three_fields(self, db_service):
        ...
        result = db_service.get_table_data('users', rql_query='select(name,email,department)')
...
class TestRQLComplexQueries:
    ...
    def test_and_condition(self, db_service):
        """Test AND logic."""
        result = db_service.get_table_data('users', rql_query='and(eq(active,true),eq(department,Engineering))')
    ...
    def test_filter_and_sort(self, db_service):
        ...
        result = db_service.get_table_data('users', rql_query='and(eq(active,true),sort(age))')
    ...
    def test_select_filter_sort(self, db_service):
        ...
        result = db_service.get_table_data('users', rql_query='and(eq(active,true),select(name,age),sort(age))')
...
class TestPagination:
    ...
    def test_rql_with_pagination(self, db_service):
        ...
        result = db_service.get_table_data('users', rql_query='eq(active,true)', limit=2)
...
class TestCRUDOperations:
    ...
    def test_update_records(self, db_service):
        ...
        # Verify the update
        result = db_service.get_table_data('users', rql_query='eq(department,Software Engineering)')
        assert len(result['data']) > 0
    ...
    def test_delete_records(self, db_service):
        ...
        # Then delete it
        deleted_count = db_service.delete_records('users', filters={'id': created['id']})
